  ...
  try {
    const items = await query(`
      select
        ...
        count(*) filter (where overall_status = 'valid')::int as valid_count,
        count(*) filter (where overall_status = 'invalid')::int as invalid_count,
        count(*) filter (where overall_status = 'unknown' or overall_status is null)::int as unknown_count,
      ...
      group by provider, provider_type, asn
      order by provider;
    `);
...
app.get("/api/bgp/status/:provider", async (req, res, next) => {
  try {
    const items = await query<LatestBgpStatus>(
      `
      select *
...
app.get("/api/bgp/history/:provider", async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit ?? 100), 500);
...
app.get("/api/bgp/history/:provider/prefix", async (req, res, next) => {
  try {
    ...
    if (!prefix) {
      return res.status(400).json({
        ...
        message: "Please provide a prefix query parameter, for example ?prefix=102.220.176.0/22"
      });
    }
    ...
    const items = await query(
      ...
      limit $3
      `,
      [`%${req.params.provider}%`, prefix, limit]
    );
...
app.get("/api/bgp/changes/:provider", async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit ?? 100), 500);
...
app.get("/api/bgp/incidents", async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit ?? 100), 500);
...
app.get("/api/bgp/incidents/open", async (_req, res, next) => {
  try {
    const items = await query(`
      select *
      from latest_bgp_incidents
...
app.get("/api/bgp/incidents/:provider", async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit ?? 100), 500);
...
app.get("/api/dashboard/bgp", async (_req, res, next) => {
  try {
    ...
    const recentChecks = await query(`
      select
        ...
        checked_at
      from latest_bgp_status
      order by checked_at desc nulls last, provider, prefix
      limit 50;
    `);

    const providerTotals = {
      total: summary.length,
      valid: summary.filter((item: any) => item.overall_status === "valid").length,
      invalid: summary.filter((item: any) => item.overall_status === "invalid").length,
      unknown: summary.filter((item: any) => item.overall_status === "unknown").length
    };
...
app.listen(config.API_PORT, () => {
  console.log(`FibrePulse API listening on port ${config.API_PORT}`);
});
